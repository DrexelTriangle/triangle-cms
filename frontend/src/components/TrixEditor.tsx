...
function TrixEditor({ value, onChange }: TrixEditorProps) {
  ...
  useEffect(() => {
    ...
    const uploadFile = (attachment: TrixAttachment, file: File) => {
      ...
      xhr.onload = () => {
        ...
        try {
          ...
          failAttachment(
            attachment,
            `Could not add ${file.name}: unexpected server response.`,
          );
        }
      };
      ...
      xhr.onerror = () => {
        failAttachment(
          attachment,
          `Could not add ${file.name}: the upload did not reach the server.`,
        );
      };
    ...
    const handleAttachmentAdd = (event: Event) => {
      ...
      const url =
        typeof attachment.getAttribute("url") === "string"
          ? String(attachment.getAttribute("url"))
          : "";
      if (!url) return;
  ...
  const openBlockForImage = useCallback(() => {
    const editor = editorRef.current;
    if (!editor) return;
    const { composition } = editor.editor;

    const blockAtCaret = () => {
      const doc = editor.editor.getDocument();
      const [caret] = editor.editor.getSelectedRange();
      const { index } = doc.locationFromPosition(caret);
      return { doc, index, block: doc.getBlockAtIndex(index) };
    };

    if (!blockAtCaret().block?.isEmpty()) composition.insertBlockBreak();

    const { doc, index } = blockAtCaret();
    if (!doc.getBlockAtIndex(index + 1)?.isEmpty()) {
      const [imagePosition] = editor.editor.getSelectedRange();
      composition.insertBlockBreak();
      editor.editor.setSelectedRange([imagePosition, imagePosition]);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const handleFileAccept = (event: Event) => {
      ...
      openBlockForImage();
      editor.editor.insertAttachment(
        Trix.Attachment.attachmentForFile((event as TrixFileAcceptEvent).file),
      );
    };

    editor.addEventListener("trix-file-accept", handleFileAccept);
    return () => {
  ...
  useEffect(() => {
    ...
    const findDropIndex = (
      ...
      if (fromIndex >= 0) {
        if (clientY < originY) last = fromIndex - 1;
    ...
    const blockIndexOf = (figure: HTMLElement): number => {
      ...
      return block
        ? (Array.from(editor.children) as HTMLElement[]).indexOf(block)
        : -1;
    };
    ...
    const commitMove = (figure: HTMLElement, target: DropTarget) => {
      ...
      const serialized = Array.from(
        new DOMParser().parseFromString(editor.value, "text/html").body
          .children,
      );
      ...
      const Trix = window.Trix;
      if (!Trix) return;
      const movedDocument = Trix.HTMLParser.parse(
        serialized.map((block) => block.outerHTML).join(""),
        {
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          editAttachmentForFigure(
            editor.children[insertAt]?.querySelector("[data-trix-id]"),
          );
        });
      });
    };
    ...
    const beginDrag = (figure: HTMLElement, downEvent: PointerEvent) => {
      ...
      const updateDropTarget = () => {
        const index = findDropIndex(lastClientY, blockIndexOf(figure), startY);
        if (index === null) {
          dropIndicator.style.display = "none";
          dropTarget = null;
          return;
      ...
      const autoScrollStep = () => {
        ...
        if (delta !== 0) {
          const speed = Math.min(
            AUTO_SCROLL_MAX_PX,
            (Math.abs(delta) / AUTO_SCROLL_EDGE_PX) * AUTO_SCROLL_MAX_PX,
          );
    ...
    // field. Running beginDrag over them instead is what made the caption
    // impossible to click into: it calls preventDefault, and its mouseup pulls
    // focus back to the editor body.
    const TRIX_ATTACHMENT_CHROME =
      "figcaption, .attachment__caption, .attachment__toolbar";
    ...
    const onBeforeToolbar = (event: Event) => {
      ...
      const figure = editor.querySelector<HTMLElement>(
        `[data-trix-id="${String(attachment.id)}"]`,
      );
      ...
      if (url) {
        ...
        copyButton.addEventListener("mousedown", (mouseEvent) => {
          mouseEvent.preventDefault();
        });
        copyButton.addEventListener("click", (clickEvent) => {
          ...
          void copyText(url).then((copied) => {
            if (!copied) {
              setNotice(
                "Could not copy the image URL. Your browser blocked clipboard access.",
              );
              return;
            }
    ...
    const onDragStart = (event: DragEvent) => {
      if (
        gestureActive ||
        (event.target as HTMLElement | null)?.closest(".attachment--preview")
      ) {
        event.preventDefault();
      }
    };
    ...
    return () => {
      ...
      editor.removeEventListener(
        "trix-attachment-before-toolbar",
        onBeforeToolbar,
      );
      ...
      <trix-toolbar id={toolbarId}>
        <div className="trix-button-row">
          <span className="trix-button-group trix-button-group--text-tools">
            <button
              type="button"
              className="trix-button trix-button--icon-bold"
              data-trix-attribute="bold"
              data-trix-key="b"
              title="Bold"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-bold`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-italic"
            ...
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-italic`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-strike"
              data-trix-attribute="strike"
              title="Strikethrough"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-strike`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-link"
          ...
          <span className="trix-button-group trix-button-group--block-tools">
            <button
              type="button"
              className="trix-button trix-button--icon-heading-1"
              data-trix-attribute="heading1"
              title="Heading"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-heading`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-quote"
              data-trix-attribute="quote"
              title="Quote"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-quote`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-code"
              data-trix-attribute="code"
              title="Code"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-code`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-bullet-list"
              data-trix-attribute="bullet"
              title="Bullets"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-bullet`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-number-list"
              data-trix-attribute="number"
              title="Numbers"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-number`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-decrease-nesting-level"
              data-trix-action="decreaseNestingLevel"
              title="Decrease indent"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-decrease`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-increase-nesting-level"
              data-trix-action="increaseNestingLevel"
              title="Increase indent"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-increase`} />
              </svg>
            </button>
          </span>
          <span className="trix-button-group trix-button-group--file-tools">
            ...
            <button
              type="button"
              className="trix-button trix-button--icon-image"
              onClick={openPicker}
              title="Insert image from library"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-image`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-attach"
              data-trix-action="attachFiles"
              title="Attach files"
              tabIndex={-1}
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-attach`} />
              </svg>
            </button>
          </span>
          <span className="trix-button-group trix-button-group--history-tools">
            <button
              type="button"
              className="trix-button trix-button--icon-undo"
            ...
            >
              <svg className="trix-icon" aria-hidden="true">
                <use href={`#${toolbarId}-undo`} />
              </svg>
            </button>
            <button
              type="button"
              className="trix-button trix-button--icon-redo"
